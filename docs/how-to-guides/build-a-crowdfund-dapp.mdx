---
sidebar_position: 22
title: Build a Crowdfund DApp
description: Learn how to build a Crowdfund DApp with Soroban.
---

# Creating a Stellar-based Crowdfunding DApp with Soroban

![Screenshot of the Example Dapp](../../static/img/crowdfund-screenshot.png)
...
In the context of our Crowdfunding dApp, understanding how to build Docker images is crucial. The Docker images serves as the basis for our container, which provide the environment for our dApp to run.
...
This section elaborates on how our frontend can interact with our crowdfund contract, access chain data, and connect to our freighter wallet. We will accomplish this by utilizing libraries provided by the [`@soroban-react`](https://soroban-react.gitbook.io/index/), a simple, powerful framework for building modern Soroban dApps using React created by a member of the Soroban community.
...
The frontend application communicates with this RPC endpoint, initiating requests that can pull data from the network, interact with smart contracts, or execute transactions. In the context of our crowdfunding dapp, these transactions can vary from invoking functions within the existing crowdfunding smart contract such as minting tokens, contributing to a campaign or withdrawing funds. Additionaly, The frontend is used to connect to the users [Freighter wallet](https://www.freighter.app), which is used to securly sign and submit transactions.
...
Lets take a look at the [`soroban-example-dapp/pages/index.tsx`](https://github.com/stellar/soroban-example-dapp/blob/main/pages/index.tsx) file:
...
![Screenshot of the Example Dapp](../../static/img/campaign.png)
...
It uses contract RPC calls to pull important info from the token and crowdfunding contracts. It's interested in things like token balance, decimals, name, symbol, along with the crowdfunding contract's deadline and target amount.

![Pledge](../../static/img/pledge.png)

Lets breakdown the `Pledge` component into its constituent parts:

#### Progress Bar

Data is presented two ways. First, a progress bar displays the current contribution status relative to the target goal.
...
Minting is the process of creating new tokens. Within our crowdfunding application, we've implemented a feature allowing users to to mint new EXT tokens before they pledge them.
...
# Removing previous soroban-preview container
...
Now that we've explored the code for our crowdfunding dapp, let's run it and see it in action.

> Note: This section can also be found in the Example Dapp [README](https://github.com/stellar/soroban-example-dapp#readme)

## Getting Started

Before running the example decentralized app (Dapp), make sure to install the following dependencies:

- [soroban-cli v0.8.0](https://soroban.stellar.org/docs/getting-started/setup#install-the-soroban-cli)
- Docker (both Standalone and Futurenet backends require it).
- Node.js v17
- [Freighter wallet v2.9.1](https://github.com/stellar/freighter/releases/tag/2.9.1). After downloading, enable "Experimental Mode" in the settings (gear icon).

### Build the Soroban-Preview Docker Image
...
1.Run in localnet/standalone mode

2.Run on Futurenet

#### Option 1: Localnet/Standalone

If you didn't yet, build the soroban-preview docker image:
...
...
 ...
 => => writing image sha256:c79f466bd1097efe67adf67b95dcf202676fa8a5ffde186020860c4278f3c08d
 => => naming to docker.io/library/soroban-preview:9    
```

In one terminal, run the backend docker containers and wait for them to start:
...
```bash
stellar-core: Synced!; Publishing 1 queued checkpoints [23-23]: Ready to run: prepare-snapshot
stellar-core: Synced!; Publishing 1 queued checkpoints [23-23]: Waiting: prepare-snapshot
stellar-core: Synced!
```
...
Token wrapped succesfully with TOKEN_ID: 46c9828f7b2c8e4b32f8d6a330ca277985c9e49119f508aff2a217efe28af550
Token Address converted to StrKey contract address format: CBDMTAUPPMWI4SZS7DLKGMGKE54YLSPESEM7KCFP6KRBP37CRL2VAB3H
...
-rwxr-xr-x@ 1 julianmartinez  staff  5974 Jun  1 13:10 soroban_crowdfund_contract.wasm
Deploy the crowdfund contract
Contract deployed succesfully with ID: 30cd9f5b2cefaf7840ff95371ffa8a53d421faf3923ee520b55b52c2ce2ab22b
...
Add some Standalone network lumens to your Freighter wallet. Copy the address for your freighter wallet and visit: http://localhost:8000/friendbot?addr=youraddress

#### Option 2: Futurenet

Run the backend docker container with ./quickstart.sh futurenet, and wait for it to start.
...
- Name: Futurenet Local
- RPC URL: http://localhost:8000/soroban/rpc
- Passphrase: Test SDF Future Network ; October 2022
- Allow HTTP connection: Enabled
- Switch to this network: Enabled

Add some Futurenet network lumens to your Freighter wallet. Visit https://laboratory.stellar.org/#create-account, and follow the instructions to create your freighter account on Futurenet.
...
In this tutorial, we've explored the process of building a crowdfunding dapp on Soroban. We've covered the basics of smart contract development, frontend development, and wallet integration. We've also examined the code for our crowdfunding dapp and run it locally to see it in action.

We hope this tutorial has been helpful in understanding the process of building a dapp on Soroban. If you have any questions, or feedback feel free to reach out to us on [Discord](https://discord.com/channels/897514728459468821/1037073682599780494)
